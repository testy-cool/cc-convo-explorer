...
class CliMetadataTests(unittest.TestCase):
    def test_no_arguments_in_a_tty_runs_the_inline_picker(self):
        old_argv = sys.argv
        sys.argv = ["agentconvos"]
        ...
        try:
            with (
                ...
                patch.dict(
                    os.environ,
                    {"AGENTCONVOS_PICKER": "/tmp/agentconvos-pick"},
                    clear=False,
                ),
                patch("subprocess.run", return_value=completed) as run_command,
                patch("agentconvos.app.ConvoExplorer.run") as run_textual,
            ):
                main()
        finally:
            sys.argv = old_argv

        run_command.assert_called_once_with(["/tmp/agentconvos-pick"], check=False)
        run_textual.assert_not_called()

    def test_pick_command_runs_the_same_inline_picker(self):
        old_argv = sys.argv
        sys.argv = ["agentconvos", "pick"]
        completed = type("Completed", (), {"returncode": 0})()
        try:
            with (
                patch.dict(
    ...
    def test_tui_command_runs_the_explicit_full_screen_browser(self):
        old_argv = sys.argv
        sys.argv = ["agentconvos", "tui"]
        completed = type("Completed", (), {"returncode": 0})()
        try:
            with (
                patch.dict(
                    os.environ,
                    {"AGENTCONVOS_TUI": "/tmp/agentconvos-tui"},
                    clear=False,
                ),
                patch("subprocess.run", return_value=completed) as run_command,
                patch("agentconvos.app.ConvoExplorer.run") as run_textual,
        ...
        run_command.assert_called_once_with(["/tmp/agentconvos-tui"], check=False)
        run_textual.assert_not_called()

    def test_no_arguments_without_a_usable_tty_prints_help(self):
        old_argv = sys.argv
        sys.argv = ["agentconvos"]
        stream = io.StringIO()
        try:
